...
import { GoalCompleteCheck } from './GoalCompleteCheck'

export function GoalHeader({ title, fromLabel, complete, weeklyTime }: { title: string; fromLabel?: string; complete?: boolean; weeklyTime?: string }) {
  return (
    <div className="flex flex-col gap-xs">
      ...
      <div className="mt-xxs flex flex-wrap items-center gap-md text-sm text-ink-subdued">
        ...
        <span className="flex items-center gap-xs">
          ...
          {weeklyTime ? (
            <span className="font-bold text-ink">{weeklyTime}</span>
          ) : (
            <>
